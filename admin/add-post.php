...
<div x-data="postEditor()" class="pb-12">
    ...
    <form action="handlers/post_handler.php" method="POST" enctype="multipart/form-data" @submit="validateForm">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6 bg-gray-100 dark:bg-slate-900 py-4 transition-colors">
            <div>
                <h2 class="text-2xl font-black text-slate-800 dark:text-white tracking-tight">Write New Article</h2>
            ...
            <div class="flex gap-3 w-full sm:w-auto">
                <button type="submit" name="status" value="draft" class="flex-1 sm:flex-none px-6 py-2.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 font-bold rounded-lg shadow-sm hover:bg-slate-50 transition active:scale-95">
                    Save Draft
                </button>
                <button type="submit" name="status" value="published" class="flex-1 sm:flex-none px-8 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-lg shadow-lg shadow-indigo-200 dark:shadow-none transition transform active:scale-95 flex items-center justify-center">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <input type="text" name="title" x-model="title" @input="generateSlug" required
                        ...
                        class="w-full text-xl font-bold px-4 py-3 rounded-lg border border-slate-400 dark:border-slate-600 focus:ring-2 focus:ring-indigo-500 dark:bg-slate-900 dark:text-white placeholder-slate-400 outline-none transition-all">
                    ...
                    <div class="mt-4 p-2 bg-slate-50 dark:bg-slate-900/50 rounded border border-slate-100 dark:border-slate-700 flex items-center gap-2">
                        ...
                        <div class="flex-1 flex items-center gap-2 overflow-hidden">
                            <template x-if="!isSlugEditable">
                                <span class="text-xs font-mono text-indigo-600 truncate" x-text="slug || 'your-post-url-here'"></span>
                            </template>
                            <template x-if="isSlugEditable">
                    ...
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="meta_title" x-model="metaTitle" maxlength="70"
                                class="w-full text-sm rounded-lg border border-slate-400 px-4 py-2.5 dark:bg-slate-900 dark:border-slate-600 dark:text-white focus:ring-1 focus:ring-indigo-500 outline-none">
                        </div>
                        <div>
                            <div class="flex justify-between mb-2">
                            ...
                            <textarea name="meta_description" x-model="metaDesc" maxlength="200" rows="3"
                                class="w-full text-sm rounded-lg border border-slate-400 px-4 py-2.5 dark:bg-slate-900 dark:border-slate-600 dark:text-white focus:ring-1 focus:ring-indigo-500 outline-none"></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <span x-show="imgError" x-text="imgError" x-cloak class="text-[9px] text-red-500 font-bold bg-red-50 px-2 py-0.5 rounded border border-red-100"></span>
                    </div>
                    ...
                    <div x-show="imageSource === 'upload'" x-cloak>
                        <div class="border-2 border-dashed rounded-xl p-4 text-center transition cursor-pointer relative overflow-hidden"
                            :class="imgError ? 'border-red-300 bg-red-50/20' : 'border-slate-300 dark:border-slate-600 hover:bg-slate-50 dark:hover:bg-slate-700/30'"
                            @click="$refs.fileInput.click()">
                            <div x-show="imgPreview && imageSource === 'upload'" class="relative group aspect-[4/3] w-full overflow-hidden rounded-lg shadow-md border border-slate-200 dark:border-slate-700">
                                <img :src="imgPreview" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">
                                <div class="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col items-center justify-center text-white gap-3">
                                    <div class="flex items-center gap-2 bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/20">
                                        <i class="fa-solid fa-camera text-xs"></i>
                                        <span class="text-[11px] font-bold uppercase tracking-wider">Change Image</span>
                                    </div>
                                    <button type="button" @click.stop="removeImage"
                                        class="flex items-center gap-1.5 text-[10px] bg-red-600 hover:bg-red-700 px-4 py-2 rounded-full font-bold shadow-lg transition-colors active:scale-95">
                            ...
                            <div x-show="!imgPreview || imageSource !== 'upload'" class="py-6">
                                <i class="fa-regular fa-image text-4xl mb-3" :class="imgError ? 'text-red-300' : 'text-slate-300'"></i>
                                <p class="text-xs font-bold text-slate-500 dark:text-slate-400">Click to upload image</p>
                                <div class="mt-3 flex flex-wrap justify-center gap-2">
                                    <span class="text-[9px] font-black px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 rounded uppercase">800x450 px</span>
                                    <span class="text-[9px] font-black px-2 py-0.5 bg-slate-100 dark:bg-slate-700 text-slate-500 rounded uppercase">MAX 100 KB</span>
                                </div>
                            </div>
                    ...
                    <div x-show="imageSource === 'url'" x-cloak class="space-y-3">
                        ...
                        <p class="text-[10px] text-slate-400">Use a full <strong>https://</strong> link to an image hosted on CDN, Drive, Imgur, Cloudinary, etc.</p>
                        <div x-show="imgPreview && imageSource === 'url'" class="relative group aspect-[4/3] w-full overflow-hidden rounded-lg shadow-md border border-slate-200 dark:border-slate-700">
                ...
                <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <div class="space-y-4">
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-1">Select Category <span class="text-red-500">*</span></label>
                            <select name="category_id" required class="w-full rounded border border-slate-400 px-3 py-2.5 text-sm font-bold dark:bg-slate-900 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-indigo-500">
                        ...
                        <div>
                            ...
                            <input type="text" name="tags" placeholder="e.g. India, Sports, Cricket"
                ...
                <div class="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700">
                    ...
                    <textarea name="summary" rows="4" maxlength="300" placeholder="Briefly explain the news..."
                        class="w-full rounded border border-slate-400 px-4 py-3 text-sm dark:bg-slate-900 dark:border-slate-600 dark:text-white outline-none focus:ring-1 focus:ring-indigo-500"></textarea>
                </div>
    ...
    function postEditor() {
        return {
            ...
            validateImage(e) {
                ...
                if (file.size > 102400) {
                    this.imgError = "Too heavy! Max 100KB";
                    this.removeImage();
                    return;
                }
                ...
                img.onload = () => {
                    if (img.width < 800 || img.height < 450) {
                        this.imgError = `Low Res! Min 800x450 (Current: ${img.width}x${img.height})`;
                        this.removeImage();
                    } else {
